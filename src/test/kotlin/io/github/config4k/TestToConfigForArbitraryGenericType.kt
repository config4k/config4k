...
class TestToConfigForArbitraryGenericType :
    WordSpec({
        ...
        "TwoPetPerson.toConfig" should {
            "return Config having TwoPetPerson<Dog, Cat>" {
                ...
                person.extract<TwoPetPerson<Dog, Cat>>("person") shouldBe
                    TwoPetPerson(
                        "foo",
                        20,
                        Dog("woof"),
        ...
        "PetPerson<HungrySnake<Mouse>>.toConfig" should {
            "return Config having PetPerson<Mouse>" {
                val person =
                    PetPerson("bar", 42, HungrySnake(3, listOf(Mouse("foobar")))).toConfig("person")
                person.extract<PetPerson<HungrySnake<Mouse>>>("person") shouldBe
                    PetPerson(
                        "bar",
                        42,
                        HungrySnake(3, listOf(Mouse("foobar"))),
                    )
            }
        }
        ...
        "TwoPetSwapPerson<Dog, Cat>.toConfig" should {
            "return Config having TwoPetSwapPerson<Dog, Cat>" {
                ...
                person.extract<TwoPetSwapPerson<Dog, Cat>>("person") shouldBe
                    TwoPetSwapPerson(
                        "Jon",
                        21,
                        Cat(9),
