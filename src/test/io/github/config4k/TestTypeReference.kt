...
internal class TestTypeReference : WordSpec({
    "TypeReference.genericType" should {
        ...
        "find container type in list"{
            val clazz = ClassContainer(GenericTestClass::class, mapOf("T1" to ClassContainer(Long::class), "T2" to ClassContainer(Float::class)))
        ...
        "find nested container type in map"{
            ...
            genericType shouldBe mapOf("K" to ClassContainer(String::class), "V" to ClassContainer(List::class, mapOf("E" to ClassContainer(Long::class))))
        }

        "throw exception when clazz container type arguments don't match"{
            val clazz = ClassContainer(GenericTestClass::class, mapOf("T1" to ClassContainer(Long::class), "T" to ClassContainer(Float::class)))
            val constructor = GenericTestClass::class.primaryConstructor!!
            val param = constructor.parameters[2] // List<Float>
            val type = param.type.javaType as ParameterizedType
...
data class GenericTestClass<T1: Number, T2: Number>(val number1: T2,
                                                    val number2: T1,
                                                    val list: List<T2>,
